...
const EditMarks = () => {
  ...
  const fetchData = async () => {
    try {
      const [studentsRes, subjectsRes, examsRes, resultRes] = await Promise.all([
        api.get('/students/'),
        api.get('/subjects/'),
        api.get('/exams/'),
        api.get(`/results/${id}/`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.put(`/results/${id}/`, {
      ...
      alert('Failed to update marks: ' + JSON.stringify(error.response?.data));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="content-area">
      <h1 style={{ color: '#1976d2', marginBottom: '20px' }}>Edit Marks</h1>
      ...
      <div className="card" style={{ maxWidth: '600px' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
            ...
            <button type="button" className="btn" style={{ background: '#f44336', color: 'white' }} onClick={() => navigate('/results')}>
              Cancel
            </button>
          </div>
        </form>
